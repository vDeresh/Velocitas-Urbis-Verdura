...
double acceleration(double drag, double tyreWear, double mass, double downforce,
                    double distanceToCarAhead, double speedOfCarAhead, double downforceOfCarAhead, double speed, float wasOvertaken)
{
    ...
    return ((23 - (drag - 2 * pow(2 + tyreWear, 2)) - ((90 * downforce + mass) / 360)) /*+ slipstream(distanceToCarAhead, speedOfCarAhead, downforceOfCarAhead, speed, wasOvertaken)*/) / 2 / FPS;
}
