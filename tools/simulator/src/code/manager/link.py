...
def calculate_speed(alreadyTurning: int, currentSpeed: float, distanceToTurn: float, tyreWear: float, tyreType: float, driversBrakingSkill: float, referenceTargetSpeed: float, mass: float, downforce: float, drag: float, distanceToCarAheadSquared: float, speedOfCarAhead: float, downforceOfCarAhead: float, wasOvertaken: float) -> float:
    return physics.handleSpeed(ctypes.c_int(alreadyTurning), ctypes.c_double(currentSpeed), ctypes.c_double(distanceToTurn), ctypes.c_double(tyreWear), ctypes.c_double(tyreType), ctypes.c_double(driversBrakingSkill), ctypes.c_double(referenceTargetSpeed), ctypes.c_double(mass), ctypes.c_double(downforce), ctypes.c_double(drag), ctypes.c_double(distanceToCarAheadSquared), ctypes.c_double(speedOfCarAhead), ctypes.c_double(downforceOfCarAhead), ctypes.c_float(wasOvertaken))
...
def calculate_slipstream_multiplier(distanceToCarAheadSquared: float, speedOfCarAhead: float, downforceOfCarAhead: float, wasOvertaken: float) -> float:
    return physics.slipstreamMultiplier(ctypes.c_double(distanceToCarAheadSquared), ctypes.c_double(speedOfCarAhead), ctypes.c_double(downforceOfCarAhead), ctypes.c_float(wasOvertaken))
